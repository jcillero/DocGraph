...
- size: 9643 bytes
- sha256: `ea0320656cab1bb333dab82bed00c15c0e9efa3203ad8e13b93e023d387c063a`
...
def fit_banner_text(text: str, width: int) -> str:
    text = " ".join(text.split())  # normaliza espacios
    if len(text) <= width:
        return text.center(width)
    # recorta con elipsis sobria
    return (text[: width - 1] + "…")
...
def banner(title: str, subtitle: str = "", inner_width: int = 44) -> str:
    # Normaliza
    ...
    if len(title) > inner_width:
        ...
        title = title.center(inner_width)
...
def shell_mode(app_root: Path) -> int:
    while True:
        ...
        # El banner se encarga de recortar/centrar y mantener el marco perfecto
